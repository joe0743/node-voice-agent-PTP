...
- `minimum-release-age=14400` - Packages must be 10+ days old before installation (4 hours in minutes)
...
- `symlink=true` - Use symlinks for performance optimization
- `frozen-lockfile=false` - Allow lockfile updates during local development
...
The lockfile (`pnpm-lock.yaml`) is protected:
